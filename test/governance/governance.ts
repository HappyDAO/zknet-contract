...
describe("Governance", function() {
  ...
  async function init() {
    ...
    logger.info(await runtime.providerL1.getBlockNumber());
    governanceToken = await runtime.deployGovernanceToken([tokenSupply]);
    logger.info(`Deployed Governance token contract : ${governanceToken.address}`);
    //  //GovernorSettings(0 /* 1 block */, 45818 /* 1 week */, 0)
    governance = await runtime.deployGovernance([governanceToken.address, 0, 15, 0]);
  ...
  it("2. Vote on a proposal", async function() {
    ...
    governance.on("VoteCast", (account, pid, support, weight, reason, event) => {
      logger.info(`account ${account}, pid: ${pid}, support: ${support}, weight: ${weight}, reason: ${reason}, event: ${event}`);
    });
  ...
  it("3. Execute the proposal", async function() {
    ...
    // const queueTx = await governance.q;
    logger.info(await runtime.providerL1.getBlockNumber());
    // await governance.setVotingPeriod(0);

    const executeTx = await governance.execute(
      [tokenAddr],
      [0],
      [transferCalldata],
      ethers.utils.id(desc)
    );
