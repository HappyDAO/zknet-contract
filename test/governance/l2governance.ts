...
describe("L2Governance", function () {
  ...
  it("1. Submit l1-l2 proposal", async function () {
    ...
    const baseCost = 1000000000000;
    const invokeAddr = governance.address;
    const invokeData = governance.interface.encodeFunctionData("callZkSync", [
      zkSyncAddress,
      counter.address,
      data,
      ergsLimit,
    ...
    const proposeTx = await governance.propose([invokeAddr], [baseCost], [invokeData], desc);
    ...
    const hashId = await governance.hashProposal([invokeAddr], [baseCost], [invokeData], utils.id(desc));
  ...
  it("3. Execute l1-l2 proposal", async function () {
    ...
    // TODO fix me : const baseCost = await zkSyncContract.executeBaseCost(gasPrice, ergsLimit, ethers.utils.arrayify(data).length, 0, 0);
    //

    logger.info(`the deployer's wallet balance: ${await runtime.deployWallet.ethWallet().getBalance()}`);
    ...
    const executeTx = await governance.execute([invokeAddr], [baseCost], [invokeData], ethers.utils.id(desc));
    await executeTx.wait();
