...
contract Vault is IVault, Base {
    function balance(uint32 token) public view returns (int256) {
        return _balance[msg.sender][token];
    }

    function bind(SignedAccount calldata l1Account) external override {}

    function deposit(uint32 token, uint256 amount)
        external
        override
        nonReentrant
    {
    ...
    function withdraw(uint32 token, uint256 amount)
        external
        override
        nonReentrant
    {
    ...
    function positionDeposit(
        ...
        if (position.id == 0) {
            ...
            require(
                position.owner == _msgSender(),
                "Vault: not position owner"
            );
            require(
                position.marginToken == token,
                "Vault: margin token not match"
            );
            position.marginAmount += int256(amount);
        }
    ...
    function positionWithdraw(
        ...
        Types.Position memory position = _getAndCheckPosition(
            positionId,
            _msgSender()
        );
        ...
        require(
            position.marginAmount >= int256(amount),
            "Vault: insufficient margin balance"
        );
